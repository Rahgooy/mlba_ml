...
percent=30
fitParams = extractFitParams(paste("MLBA-E4-BayesHierFit-5000-", percent, "-percent.Rdata", sep=""))
e3a.pred = pred("../Data/parsedData-E3a.Rdata", 2000)
e3b.pred = pred("../Data/parsedData-E3b.Rdata", 2000)
e3c.pred = pred("../Data/parsedData-E3c.Rdata", 2000)

exportJSON <- toJSON(e3a.pred)
write(exportJSON, paste("e3a.", percent, "percent.json", sep = ""))

exportJSON <- toJSON(e3b.pred)
write(exportJSON, paste("e3b.", percent, "percent.json", sep = ""))

exportJSON <- toJSON(e3c.pred)
write(exportJSON, paste("e3c.", percent, "percent.json", sep = ""))

#mesh = mesh_data(1000, 0.05)  
#write.csv(mesh, 'mesh_E4.csv')

#savefile=paste("MLBA-E4-pred-E3",fitParams$nmc,sep="-")
